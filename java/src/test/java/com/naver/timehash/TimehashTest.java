...
public class TimehashTest {
    private Timehash th = new Timehash();

    @Test
    public void testGetQueryTerms() {
        // Test query term generation for 14:30
        List<String> terms = th.getQueryTerms(1430);
        assertEquals(5, terms.size());
        assertEquals("12", terms.get(0));
        assertEquals("1214", terms.get(1));
        assertEquals("121430", terms.get(2));
        // Level 4 (5-minute): 14:30 is in the 5-minute block starting at 14:30, so key is 30
        assertEquals("12143030", terms.get(3));
        // Level 5 (1-minute): 14:30 is exactly at minute 30, so key is 30
        assertEquals("1214303030", terms.get(4));
    }

    @Test
    public void testGetQueryTermsInvalid() {
        // Test with invalid input (> 10000)
        List<String> terms = th.getQueryTerms(10001);
        assertEquals(0, terms.size());
    }

    @Test
    public void testGetIndexTermsSimple() {
        // Test index term generation for 12:00 - 13:00
        List<String> terms = th.getIndexTerms("1200", "1300");
        assertTrue("Should contain key 1212", terms.contains("1212"));
    }

    @Test
    public void testGetIndexTermsComplex() {
        // Test index term generation for 11:40 - 21:00
        List<String> terms = th.getIndexTerms("1140", "2100");
    ...
    @Test
    public void testQueryMatchesIndex() {
        // Business: 11:40 - 21:00
        List<String> indexTerms = th.getIndexTerms("1140", "2100");
        
        // Query: 14:30 (should match)
        List<String> queryTerms = th.getQueryTerms(1430);
        boolean hasMatch = queryTerms.stream().anyMatch(indexTerms::contains);
        assertTrue("Query at 14:30 should match business open 11:40-21:00", hasMatch);
        
        // Query: 10:00 (should not match)
        List<String> queryTerms10 = th.getQueryTerms(1000);
        boolean hasMatch10 = queryTerms10.stream().anyMatch(indexTerms::contains);
